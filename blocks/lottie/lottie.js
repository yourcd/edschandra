 ...
 * Pattern from https://www.getswivel.io/ (lottie-animation, viewBox 610×352).
 * Authors set animation path/URL in block table:
 *   animation | /blocks/lottie/lottie.json (or full URL).
 *
 * EDS: Uses lottie-web SVG renderer only (no <lottie-player> — Shadow DOM/WASM fails on EDS CSP).
 * AE expressions (e.g. loopOut('cycle')) are expanded to keyframes so the expression
 * evaluator is not used.
 * Debug: ?lottie=immediate for immediate load; window.lottie.getRegisteredAnimations()
 * to confirm load.
 */
...
function loadLottieIntoContainer(container) {
  ...
  const isSwivelStyle = container.classList.contains('lottie-animation');
...
function getDefaultSwivelJsonUrl() {
  const base = (typeof window !== 'undefined' && window.hlx?.codeBasePath) ? window.hlx.codeBasePath.replace(/\/$/, '') : '';
  const path = `${base}/blocks/lottie/lottiefile.json`;
  try {
    return new URL(path, typeof window !== 'undefined' ? window.location.origin : '').href;
  } catch {
    return '/blocks/lottie/lottiefile.json';
  }
}
...
export default function decorate(block) {
  ...
  container.className = 'lottie-animation lottie-lazy lottie-container';
